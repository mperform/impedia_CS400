...
public class Graph<T> implements GraphADT<T>{
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        ...
        PriorityQueue<Path> queue = new PriorityQueue();//holds the discovered paths

        Vertex strt = vertices.get(start);//stores the starting vertex
        Vertex ed = vertices.get(end);//stores the target vertex
        if (strt==null||ed==null){//if either is null, this means the data is not within the hashtable
            throw new NoSuchElementException();
        }
        int recurCnt = 0;

        Path finalPath = new Path(strt);//this stores the shortest path as we loop thru the hashtable

        LinkedList<Vertex> visited = new LinkedList<>();//this stores all the visited vertices
        Vertex prev = null;
        Vertex cur = strt;//this stores the current vertex that we are checking in the while loop
        visited.add(cur);//this adds the current vertex to the vertices visited list

        //while the final path doesn't contain the end, keep looping
        //!finalPath.dataSequence.contains(ed.data)
        //System.out.println("Start: "+start);
        //System.out.println("End: "+end);
        while (!cur.data.equals(prev)){
            //System.out.println(cur.data);
            int before = queue.size();//this stores the size of the queue before adding the new paths
            boolean added = false;
            //loop to find all possible paths

            for (int i = 0;i<cur.edgesLeaving.size();i++) {
                //System.out.println("Cur edges leaving "+cur.edgesLeaving.get(i).target.data);
                if (cur.edgesLeaving.get(i)==null||cur.edgesLeaving.get(i).target==null){
                    throw new NoSuchElementException();
                }
                Vertex target = cur.edgesLeaving.get(i).target;//stores the targeting vertex

                if (!visited.contains(target)) {//if the target is visited, then don't add it as a path
                    queue.add(new Path(finalPath, cur.edgesLeaving.get(i)));//adds the found edge to the possible paths list
                    recurCnt=0;
                }
            }
            //System.out.println("Cur: "+cur.data);
            int after = queue.size();
            queue.remove(finalPath);//delete the current path since it has been extended already
            finalPath = queue.peek();//pulls the shortest path from the queue and store it as the shortest path
            if (finalPath==null||finalPath.end==null){
                //System.out.println("Path not possible.");
                throw new NoSuchElementException();
            }
            prev = cur;
            cur = finalPath.end;//sets the current vertex as the end of the final path
            visited.add(cur);//adds the current vertex to the visited list
            //System.out.println("cur "+cur.data+" end "+end);
            if (cur.data.equals(end)){
                return finalPath;
            }
            Path tst = queue.peek();
            //System.out.println("Current shorest path:"+tst.start.data+" to "+tst.end.data);
        }
